...
function polygon(obj){
    ...
    this.vec.push(new Vec(obj.x + obj.width * Math.cos(obj.angle), obj.y + obj.width * Math.sin(obj.angle)));   
    this.vec.push(new Vec(obj.x + obj.width * Math.cos(obj.angle) + obj.height * Math.cos(obj.angle + Math.PI / 2), obj.y + obj.width * Math.sin(obj.angle) + obj.height * Math.sin(obj.angle + Math.PI / 2)));
    this.vec.push(new Vec(obj.x + obj.height * Math.cos(obj.angle + Math.PI / 2), obj.y + obj.height * Math.sin(obj.angle + Math.PI / 2)));
    for(let i = 0; i < this.vec.length; i++){
        let dir = new Vec(this.vec[(i + 1) % this.vec.length].x - this.vec[i].x, this.vec[(i + 1) % this.vec.length].y - this.vec[i].y);
        this.edges.push(new line(this.vec[i], this.vec[(i + 1) % this.vec.length], dir));
    }
};
...
const overlap = function(a, b){
    if (contains(a[0], b) || contains(a[1], b) || contains(b[0], a) || contains(b[1], a)){
        return true;
    }
    return false;
};
